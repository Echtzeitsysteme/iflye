...
public class CostUtility {
	...
	/**
	 * Returns the total communication cost for a link to element embedding as
	 * defined in [1].
	 *
	 ...
	 */
	public static double getTotalCommunicationCostLinkA(final VirtualLink virt, final SubstrateElement host) {
	 ...
	 * @return Total communication cost for a link to element embedding.
	 */
	public static double getTotalCommunicationCostLinkA(final VirtualLink virt, final List<SubstrateElement> hosts) {
		if (hosts.size() == 1) {
	...
	/**
	 * Returns the total communication cost for a link to element embedding. In
	 * comparison to the paper [1], we define the cost of one hop as 1 as stated in
	 * [2].
	 *
	 ...
	 * @return Total communication cost for a link to element embedding.
	 */
	public static double getTotalCommunicationCostLinkBCD(final VirtualLink virt, final SubstrateElement host) {
		if (host instanceof Server) {
			// Server -> 0 hops
			return 0;
		} else if (host instanceof SubstrateLink) {
	...
	/**
	 * Returns the total communication cost for a link to list of elements
	 * embedding. In comparison to the paper [1], we define the cost of one hop as 1
	 * as stated in [2].
	 *
	 * Implementation of the cost function of paper [1] and [2].
	 *
	 ...
	 * @return Total communication cost for a link to element embedding.
	 */
	public static double getTotalCommunicationCostLinkBCD(final VirtualLink virt, final List<SubstrateElement> hosts) {
		if (hosts.size() == 1) {
			// One host object -> call other cost method for calculation
