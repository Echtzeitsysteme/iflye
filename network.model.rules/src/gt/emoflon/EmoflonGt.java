...
import network.model.rules.api.matches.LinkMatchPositiveMatch;
...
public class EmoflonGt implements IncrementalPatternMatcher {
	...
	public EmoflonGt(final Root root) {
		...
		api.linkMatchPositive().subscribeAppearing(m -> {
			addMatch(currentDelta::addLinkMatchPositive, m.getVirtualLink(), m.getSubstrateLink());
			tupleToGtMatch.put(new Tuple(m.getVirtualLink(), m.getSubstrateLink()), m);
		});
	}
	...
	public void apply(final VirtualElement virt, final SubstrateElement sub, final boolean doUpdate) {
		...
		} else if (match instanceof LinkMatchPositiveMatch) {
			api.linkMatchPositive().apply((LinkMatchPositiveMatch) match, doUpdate);
		}
	}
