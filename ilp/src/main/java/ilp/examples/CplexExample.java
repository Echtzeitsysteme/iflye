...
/**
 * Simple example to solve an optimization problem with the CPLEX solver [1]. The example is taken
 * from IBM's documentation [2,3]. This example exists to check the correct installation, licensing
 * and configuration of the CPLEX solver.
 * 
...
public class CplexExample {
  ...
  static void populateByRow(final IloMPModeler model, final IloNumVar[][] var,
      final IloRange[][] rng) throws IloException {
    ...
    rng[0][0] = model.addLe(
        model.sum(model.prod(-1.0, x[0]), model.prod(1.0, x[1]), model.prod(1.0, x[2])), 20.0);
    rng[0][1] = model.addLe(
        model.sum(model.prod(1.0, x[0]), model.prod(-3.0, x[1]), model.prod(1.0, x[2])), 30.0);
  }
