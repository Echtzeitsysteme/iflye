...
public class VneIlpPathAlgorithm extends AbstractAlgorithm {
	...
	/**
	 * Wrapper class for substrate paths. A path may consist of a server or a list
	 * of links.
	 */
	class VneIlpPath {
		...
		public boolean equals(final Object obj) {
			...
			if ((obj == null) || !(obj instanceof VneIlpPath)) {
				return false;
			}
	...
	protected final Map<VirtualLink, List<SubstrateElement>> resultVirtualToSubstrateLink = new HashMap<>();
	...
	protected void createEmbeddings() {
		// We have to check if at least one node was embedded for each virtual network.
		// If that is not
		// the case, we must not embed the network itself to keep the model consistent.
		final Set<VirtualNetwork> embedNetwork = new HashSet<>();
		...
		for (final VirtualLink vl : resultVirtualToSubstrateLink.keySet()) {
			...
			if (hosts.size() > 1) {
				...
				final SubstratePath p = facade.getPathFromSourceToTarget((SubstrateNode) links.get(0).getSource(),
						(SubstrateNode) links.get(links.size() - 1).getTarget());
				...
				if (hosts.get(0) instanceof SubstrateServer) {
					...
					final SubstratePath p = facade.getPathFromSourceToTarget((SubstrateNode) l.getSource(),
							(SubstrateNode) l.getTarget());
					facade.embedLinkToPath(p.getName(), vl.getName());
				}
			}
		}
	...
	protected void createMinOveralCostsObjective() {
		...
		for (int v = 0; v < nodeVariables.length; v++) {
			for (int s = 0; s < nodeVariables[v].length; s++) {
				expr.add(mult(param(getNodeCost(virtualNodes.get(v), substrateNodes.get(s))), nodeVariables[v][s]));
			}
		}

		for (int v = 0; v < pathVariables.length; v++) {
			for (int s = 0; s < pathVariables[v].length; s++) {
				expr.add(mult(param(getLinkCost(virtualLinks.get(v), allSubstratePaths.get(s).getLinksOrServer())),
						pathVariables[v][s]));
			}
		}
	...
	private double getLinkCost(final VirtualLink virt, final List<SubstrateElement> hosts) {
		...
		case TOTAL_COMMUNICATION_COST_A:
			return IlpSolverConfig.transformObj(CostUtility.getTotalCommunicationCostLinkA(virt, hosts));
		case TOTAL_COMMUNICATION_COST_B:
			return IlpSolverConfig.transformObj(CostUtility.getTotalCommunicationCostLinkBCD(virt, hosts));
		case TOTAL_COMMUNICATION_COST_C:
			return IlpSolverConfig.transformObj(CostUtility.getTotalCommunicationCostLinkBCD(virt, hosts));
		case TOTAL_COMMUNICATION_COST_D:
			return IlpSolverConfig.transformObj(CostUtility.getTotalCommunicationCostLinkBCD(virt, hosts));
	...
	/**
	 * Creates the network information. This method extracts all nodes and links
	 * from given substrate and virtual network as well as the substrate paths to
	 * generate.
	 */
	...
	protected void createAllNodeConstraints() {
		// Every substrate node must be able to host the service type of the virtual
		// node
		for (int v = 0; v < nodeVariables.length; v++) {
			for (int s = 0; s < nodeVariables[v].length; s++) {
		...
		// The resources of the substrate node are not over booked by the demands of all
		// virtual
	...
	protected void createAllLinkConstraints() {
		...
		for (int v = 0; v < virtualLinks.size(); v++) {
			for (int s = 0; s < allSubstratePaths.size(); s++) {
				...
				model.newConstraint(leq(pathVariables[v][s],
						getNodeVariable(virtualLink.getSource().getName(), subtratePath.getSourceNode().getName())));

				// Target to target constraint
				model.newConstraint(leq(pathVariables[v][s],
						getNodeVariable(virtualLink.getTarget().getName(), subtratePath.getTargetNode().getName())));
			}
		}
			...
			for (int s = 0; s < allSubstratePaths.size(); s++) {
				for (final SubstrateLink tempLink : allSubstratePaths.get(s).getLinks()) {
					if (tempLink.equals(subLink)) {
						for (int v = 0; v < virtualLinks.size(); v++) {
							sumBandwidthExpr.add(mult(pathVariables[v][s], param(virtualLinks.get(v).getBandwidth())));
						}
					}
				}
			}
		...
		for (int s = 0; s < allSubstratePaths.size(); s++) {
			...
			model.newConstraint(leq(sum(exprBandwidth), param(allSubstratePaths.get(s).getResidualBandwidth())));
		}
	}

	/**
	 * Adds a set of constraints which ensure that all virtual elements (nodes,
	 * links) are mapped exactly once.
	 */
	...
	/**
	 * Returns the node variable for a given virtual node name and a substrate node
	 * name.
	 *
