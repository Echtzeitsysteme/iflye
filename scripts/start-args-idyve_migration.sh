...
do
    ...
    export RUN_NAME="${a}_${s}-${p}-pods_l${l}_k1_mig_run${i}"
    mkdir -p $OUTPUT$RUN_NAME
    export ARGS="$a $p $s $l $OUTPUT$RUN_NAME/ 600 0 ALWAYS_FREE 40"
    echo "#"
    ...
    export RUN_NAME="${a}_${s}-${p}-pods_l${l}_k1_mig_run${i}_mem"
    mkdir -p $OUTPUT$RUN_NAME
    export ARGS="$a $p $s $l $OUTPUT$RUN_NAME/ 600 0 ALWAYS_FREE 40 memmeasurement"
    echo "#"
    echo "# => Using ARGS: $ARGS"
    echo "#"
    run
