...
/**
 * Evaluation scenario of paper [1]. There are some changes and assumptions: (1)
 * Every virtual network is a one tier network with exactly one switch. (2)
 * Every virtual machine of one particular virtual network has the same amount
 * of CPU, memory and bandwidth. The same applies to every link of a virtual
 * network. (3) The substrate network is a two tier network with two core
...
public class OptimalVmScenario extends IflyeLogger implements IScenario {
	...
	/**
	 * Amount of storage per substrate server. The paper [1] uses this value for I/O
	 * instead of storage.
	 */
	public static final int substrateSto = 300;

	/**
	 * Amount of bandwidth per link that connects a substrate server. This value was
	 * not explicitly given in the paper [1].
	 */
	public static final int substrateBwSrv = 1000;

	/**
	 * Amount of bandwidth for all links between rack and core switches. This values
	 * was not explicitly given in the paper [1].
	 */
	...
	private static void substrateSetup(final String substrateNetworkId) {
		final OneTierConfig substrateRackConfig = new OneTierConfig(N, 1, false, substrateCpu, substrateMem,
				substrateSto, substrateBwSrv);
	...
	private static void virtualSetup(final String virtualNetworkId) {
		...
		final OneTierConfig virtualConfig = new OneTierConfig(L, 1, false, cpu, memory, storage, bandwidth);
